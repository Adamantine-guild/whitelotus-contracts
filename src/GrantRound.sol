...
contract GrantRound {
    ...
    event MilestoneEvidenceSubmitted(uint256 indexed appId, uint256 indexed index, string evidenceURI);
    ...
    function submitApplication(string calldata uri) external returns (uint256 appId) {
        ...
        applications[appId] = Application({applicant: msg.sender, uri: uri, status: AppStatus.Pending});
    ...
    function createMilestones(uint256 appId, uint256[] calldata amounts) external onlyAdmin {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            _milestones[appId].push(
                Milestone({amount: amounts[i], evidenceURI: "", submitted: false, approved: false, paid: false})
            );
            total += amounts[i];
        }
    ...
    function submitMilestoneEvidence(uint256 appId, uint256 index, string calldata evidenceURI) external {
        Application storage app = applications[appId];
        require(app.applicant != address(0), "app !exists");
        require(app.status == AppStatus.Approved, "app !approved");
